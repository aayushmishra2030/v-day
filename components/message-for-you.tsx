...
export default function MessageForYou({ onOpen }: MessageForYouProps) {
  ...
  return (
    <div className="relative w-full max-w-md mx-auto">
      <motion.div
        className={`bg-red-100 rounded-lg shadow-lg overflow-hidden cursor-pointer ${isOpen ? "h-[80vh]" : "h-48"}`}
        onClick={handleOpen}
        layout
      >
        <motion.div
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              ...
              exit={{ opacity: 0 }}
            >
              <p className="text-red-600 text-sm font-pixel leading-relaxed">
